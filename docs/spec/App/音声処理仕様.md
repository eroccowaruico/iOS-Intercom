...
マイク側は SessionManager で Audio Session と入力 node 側の voice processing を構成したうえで、マイク入力を mixer に入れる。

| 順序 | 処理 |
|---|---|
| 1 | SessionManager で入力デバイス、Sound Isolation、他音声ダック、入力ミュートを構成する |
| 2 | マイク入力を AudioMixer の送信用 bus へ入れる |
| 3 | 送信用 bus で SoundIsolation を必要に応じて適用する |
| 4 | VADGate で発話判定と gate を行う |
| 5 | DynamicsProcessor で送信用音声の音量差を整える |
| 6 | PeakLimiter で送信用音声の過大ピークを抑える |
| 7 | 送信可能な音声を codec / RTC へ渡す |

### 受信側

受信側は peer ごとの入力を個別 bus として mixer に入れる。各 peer bus で個別音量を調整し、その後に複数の受信音声をまとめた受信 master bus で全体音量を調整する。

| 段階 | 処理 |
|---|---|
| decode | RTC から受け取った受信音声を Codec で PCM へ戻す |
| peer input | 受信した peer ごとの音声を個別 bus へ入れる |
| peer volume | peer ごとの出力音量を個別 bus で反映する |
| optional peer effect | 必要に応じて個別 peer bus に SoundIsolation を挿入できる |
| receive mix | 複数 peer bus を受信 master bus にまとめる |
| receive master volume | 受信 master bus で受信音声全体の音量を調整する |
| optional master effect | 必要に応じて受信 master bus に SoundIsolation を挿入できる |
| final output | 最終出力 bus へ渡す |

SoundIsolation は受信側では個別 peer bus、または複数受信をまとめた受信 master bus のどちらにも挿入できる。個別に挿す場合は参加者ごとの聞こえ方を調整し、全体に挿す場合は受信音声全体の聞こえ方をまとめて調整する。
