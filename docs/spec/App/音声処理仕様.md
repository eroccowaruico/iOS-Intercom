# RideIntercom App 音声処理仕様

## 目的

本書は、作り直す RideIntercom App が音声系 package をどう組み合わせるかだけを定義する。

音声処理の詳細は `docs/spec/packages` を正とし、本書では App 本体に残る責務だけを扱う。

## 責務境界

| 領域 | App 本体の責務 | 詳細仕様 |
|---|---|---|
| Audio Session | media 開始/終了に合わせて session を構成し、入出力デバイス選択を渡す | `docs/spec/packages/Audio/SessionManager.md` |
| 入力処理 | マイク権限、ミュート、他音声ダックのユーザー設定を保持する | `docs/spec/packages/Audio/SessionManager.md` |
| VAD / gate | App 設定から VAD 感度 preset を選び、送信可否や発話表示へ使う | `docs/spec/packages/Audio/Effectors/VADGate.md` |
| codec | App の codec UI 設定を Codec / RTC の codec policy へ渡せる形に変換する | `docs/spec/packages/Audio/Codec.md`、`docs/spec/packages/RTC.md` |
| mixer / output | master 音量、出力ミュート、参加者別音量を反映する | `docs/spec/packages/Audio/Mixer.md` |
| effect | App が使う effect の有効/無効を決める | `docs/spec/packages/Audio/Effectors/*.md` |
| transport | 音声 frame の送受信、packet audio、WebRTC media lifecycle は RTC に委譲する | `docs/spec/packages/RTC.md` |

## 基本方針

| 項目 | 方針 |
|---|---|
| 接続と音声 | 接続確立と media 開始を分ける。peer 認証前にマイクやスピーカーを起動しない |
| OS 差分 | iOS / macOS の Audio Session 差分は App で直接扱わず、SessionManager に渡す |
| 高頻度処理 | frame、packet、jitter、codec の詳細値は App 仕様に重複定義しない |
| 設定 | 設定値は本書に重複定義せず、`docs/spec/App/setting parameters` に集約する |
| 診断 | App は package の runtime report / event を要約し、ユーザーが状態を理解できる値だけを表示する |
| ログ | App は package 操作結果を `docs/requirements/log.md` に従って構造化ログへ変換する。高頻度 frame は通常ログに出さない |

## 設定値

音声関連の App 設定、既定値、永続化、package へ渡す値は `docs/spec/App/setting parameters/App/設定値一覧.md` を正とする。

## 音声 lifecycle

| タイミング | App の動作 |
|---|---|
| 接続準備 | Audio Session、入力監視、出力 renderer は開始しない |
| peer 認証完了 | RTC の media 開始条件を満たしたら、App の音声 pipeline を開始する |
| media 開始 | SessionManager を構成し、入力、VAD、codec、mixer / output を接続する |
| media 停止 | 入力、出力、mixer を停止し、Audio Session を inactive に戻す |
| 入力設定変更 | Duck Other Audio、ローカルマイクミュートは `AudioInputStreamCapture.updateVoiceProcessing(_:)` で即時反映する |
| effect設定変更 | Voice Isolation Effect は SoundIsolation effect chain の有効/無効として扱い、SessionManager の voice processing へは渡さない |
| codec 設定変更 | `preferredTransmitCodec` と codec option は次回送信 frame から反映し、利用不可なら fallback 結果を Diagnostics とログへ出す |
| 出力設定変更 | volume / mute は Mixer bus へ即時反映する。出力 device 変更だけは SessionManager / output renderer の report を確認する |

## Mixer 構成

App は AudioMixer 上で、送信用のマイク入力系統と受信用の参加者入力系統を分けて扱う。

### マイク入力側

マイク側は SessionManager で Audio Session、Duck Other Audio、入力ミュートを構成したうえで、マイク入力を送信用処理へ入れる。Voice Isolation Effect は SessionManager の input voice processing ではなく、SoundIsolation effect chain で扱う。

| 順序 | 処理 |
|---|---|
| 1 | SessionManager で Audio Session、入力デバイス、入力 stream format を構成する |
| 2 | `AudioInputStreamCapture` で他音声ダック、入力ミュートを初期適用してマイク入力を取得する |
| 3 | AudioMixer の送信用 bus にマイク入力を source として入れる |
| 4 | 送信用 bus の ordered effect chain を先頭から順に適用する |
| 5 | effect chain の出力を `preferredTransmitCodec` に基づき Codec / RTC adapter へ渡す |

### 送信用 effect chain

送信用 effect chain は固定分岐ではなく、App runtime の `transmitEffectChainSnapshot` が返す順序付き stage 配列として扱う。Diagnostics はこの snapshot を描画するだけで、effect の既定値や stage 配列を持たない。

| stage | package | 役割 | 初期状態 | Diagnostics 表示 |
|---|---|---|---|---|
| `soundIsolation` | SoundIsolation | 入力音声の voice isolation を必要に応じて行う | `isSoundIsolationEnabled` と runtime support から有効/無効を導出 | `pipeline-effect-stage-sound-isolation` |
| `vadGate` | VADGate | 発話判定と gate を行う。SoundIsolation の後、DynamicsProcessor の前に置く | `vadSensitivity` と直近 analysis から speech / silent を表示 | `pipeline-effect-stage-vad-gate` |
| `dynamicsProcessor` | DynamicsProcessor | 送信用音声の音量差を整える | App adapter の chain 設定を正とする | `pipeline-effect-stage-dynamics-processor` |
| `peakLimiter` | PeakLimiter | 送信用音声の過大ピークを抑える | App adapter の chain 設定を正とする | `pipeline-effect-stage-peak-limiter` |

| 追加時のルール | 仕様 |
|---|---|
| 追加位置 | effect の意味に応じて stage 配列へ挿入する。VADGate は発話判定のため SoundIsolation 後、音量整形前を標準位置とする |
| UI | Diagnostics は `transmitEffectChainSnapshot.stages` を描画する。固定の `SoundIsolation -> VADGate -> DynamicsProcessor -> PeakLimiter` 文字列に依存しない |
| 設定 | ユーザーに出す設定は App 設定値一覧に追加する。Audio Unit parameter や effect index は通常 UI に出さない |
| 異常系 | unsupported / ignored / bypassed は stage 単位で表示し、chain 全体を即 error にしない |

### 受信側

受信側は RTC / Codec で PCM に戻した peer ごとの音声を AudioMixer の peer bus に入れる。相手は 0...n 人で、peer ごとに RTC 接続状態、認証状態、decode codec、AudioMixer peer bus を別々の診断層として持つ。各 peer bus は個別音量と ordered effect chain を持ち、その後に全 peer bus を receive mix down で 1 本の receive master bus へまとめる。receive master bus は全体音量と ordered effect chain を持ち、最後に SessionManager の output renderer へ渡す。

| 段階 | 処理 |
|---|---|
| RTC receive | RTC から peer ごとの受信音声 frame / encoded frame を受け取る |
| Codec decode | peer ごとの codec metadata / active codec を確認しながら PCM へ戻し、decode 失敗は drop として扱う |
| peer buses | peer ごとの AudioMixer bus に PCM を source として入れ、peer volume を反映する。相手が複数いる場合は bus も複数並ぶ |
| peer effect chain | 各 peer bus の ordered effect chain を先頭から順に適用する |
| receive mix down | 複数 peer bus の出力を receive master bus へ mix する |
| receive master bus | mix 後の受信音声全体に master volume / mute を反映する |
| master effect chain | receive master bus の ordered effect chain を先頭から順に適用する。最後は PeakLimiter で過大ピークを抑える |
| SessionManager output | 最終 output stream へ渡し、schedule report と level を Diagnostics に反映する |

Diagnostics は受信側を単一 peer の直列処理として見せない。`RTC RX` は誰が接続・認証されているか、`Decode` は peer ごとの `activeCodec`、`Peer Buses` は RX / JIT / PLAY / level / volume / `receivePeerEffectChainSnapshot(peerID:)` を一覧し、`Mix Down` でそれらが receive master bus へ集約されることを見せる。decode codec は peer の keepalive metadata で受け取った `activeCodec` を使い、metadata 未到達時は `Unknown` として表示する。

### 受信用 effect chain

受信用 effect chain は、peer bus と receive master bus の 2 箇所にそれぞれ順序付き stage 配列として持つ。Diagnostics は `receivePeerEffectChainSnapshot(peerID:)` と `receiveMasterEffectChainSnapshot` を描画するだけで、effect の既定値や stage 配列を持たない。

| chain | stage | package | 役割 | Diagnostics 表示 |
|---|---|---|---|---|
| peer effect chain | `soundIsolation` | SoundIsolation | peer 単位で受信音声の voice isolation を必要に応じて行う | `receive-peer-{index}-effect-stage-sound-isolation` |
| master effect chain | `soundIsolation` | SoundIsolation | 複数 peer をまとめた後の受信音声全体に voice isolation を必要に応じて行う | `receive-master-effect-stage-sound-isolation` |
| master effect chain | `peakLimiter` | PeakLimiter | RX master の最終段で過大ピークを抑える。常に最後へ挿入する | `receive-master-effect-stage-peak-limiter` |

| 追加時のルール | 仕様 |
|---|---|
| 追加位置 | peer 単位で効かせたい effect は peer effect chain、受信全体へ効かせたい effect は master effect chain へ追加する。master effect chain の最後は PeakLimiter 固定 |
| UI | Diagnostics は App runtime の chain snapshot を描画する。固定の `SoundIsolation` 文字列や default 値に依存しない |
| 設定 | ユーザーに出す設定は App 設定値一覧に追加する。Audio Unit parameter や effect index は通常 UI に出さない |
| 異常系 | unsupported / ignored / bypassed は stage 単位で表示し、受信 pipeline 全体を即 error にしない |

## 他音声ダック

`Duck Other Audio` のユーザー設定、実際の有効条件、SessionManager へ渡す値は `docs/spec/App/setting parameters/App/設定値一覧.md` と `docs/spec/App/setting parameters/packages/Audio/SessionManager.md` を正とする。

OS API の適用方法、入力ノード、ducking level の詳細は `docs/spec/packages/Audio/SessionManager.md` を正とする。

## Codec UI 設定

送信 codec のユーザー設定、bitrate、fallback の扱いは `docs/spec/App/setting parameters/App/設定値一覧.md` と `docs/spec/App/setting parameters/packages/Audio/Codec.md` を正とする。

| 状態 | App の扱い |
|---|---|
| 要求 codec が利用可能 | `CodecEncodingConfiguration.codec` と `RTC.AudioCodecConfiguration.preferredCodecs` の先頭へ反映する |
| 要求 codec が現在環境で利用不可 | `pcm16` fallback が可能なら通話を継続し、Diagnostics に fallback 理由を出す |
| decode 失敗 | 該当 frame を破棄し、受信 drop として扱う |
| WebRTC route-managed media | App managed packet audio codec 設定は送信 sample に使わず、RTC / WebRTC 側の codec negotiation に任せる |

## Audio Check

Audio Check は App 本体の診断機能として残す。

| 項目 | 仕様 |
|---|---|
| 目的 | マイク入力とスピーカー出力をユーザーが単独で確認できるようにする |
| 実行条件 | 通話 media が開始していない時だけ実行する |
| 録音 | 5 秒間マイク入力を取得し、入力メーターを表示する |
| 再生 | 録音した音声を 5 秒間再生し、出力メーターを表示する |
| 失敗 | 権限拒否、入力なし、session 構成失敗をユーザーに分かる状態として表示する |
| 終了 | 一時的に使った audio pipeline を解放し、通話状態へ影響させない |

## 表示する音声状態

| 表示 | 意味 |
|---|---|
| マイク権限状態 | 未決定、拒否、許可を区別する |
| ローカル発話状態 | VAD / gate の結果から表示する |
| 入力レベル | マイク入力の現在レベルを表示する |
| 出力レベル | 最終出力へ渡した音声レベルを表示する |
| 参加者別レベル | 受信した peer ごとの音声レベルを表示する |
| Audio Check 状態 | recording、playing、completed、failed を表示する |

## 異常時

| 事象 | App の扱い |
|---|---|
| マイク権限未決定 | 権限要求中として表示し、許可後に再試行できるようにする |
| マイク権限拒否 | 設定変更が必要な状態として表示する |
| 入力/出力デバイス適用失敗 | 通信失敗とは分けて、音声デバイスの問題として表示する |
| 音声 pipeline 開始失敗 | 接続状態とは分けて、media 開始失敗として表示する |
| codec / packet / route の失敗 | App で詳細を再定義せず、RTC / Codec から受け取った error を要約表示する |

## ログ

| 事象 | Event | Level | metadata |
|---|---|---|---|
| media 開始成功 | `audio.media.started` | `.info` | `operationID`, `route`, `codec` |
| media 開始失敗 | `audio.session.failed` | `.error` | `operationID`, `errorType`, `isRecoverable` |
| 入出力 device / route 変更 | `audio.session.route_changed` | `.notice` | `operationID`, `reason` |
| voice processing 更新 ignored | `audio.input.voice_processing_ignored` | `.debug` | `operationID`, `reason` |
| voice processing 更新失敗 | `audio.input.voice_processing_failed` | `.warning` | `operationID`, `errorType`, `isRecoverable` |
| output schedule 失敗 | `audio.output.schedule_failed` | `.warning` | `operationID`, `errorType`, `isRecoverable` |
| codec fallback | `audio.codec.fallback` | `.notice` | `requestedCodec`, `selectedCodec`, `reason` |
| decode 失敗 | `audio.codec.decode_failed` | `.error` | `codec`, `errorType`, `isRecoverable` |

音声 sample、encoded payload、frame ごとの通常処理はログへ出さない。ログ仕様の正本は `docs/requirements/log.md` と `docs/spec/App/ログ仕様.md` とする。

## 移管済みとして本書に書かないもの

| 領域 | 参照先 |
|---|---|
| VAD の attack / release / noise floor | `docs/spec/packages/Audio/Effectors/VADGate.md` |
| Audio Session category / mode / option | `docs/spec/packages/Audio/SessionManager.md` |
| voice processing / advanced ducking API | `docs/spec/packages/Audio/SessionManager.md` |
| codec payload / availability / fallback | `docs/spec/packages/Audio/Codec.md` |
| mixer bus / effect chain / output format | `docs/spec/packages/Audio/Mixer.md` |
| packet audio / jitter / route-managed media | `docs/spec/packages/RTC.md` |
