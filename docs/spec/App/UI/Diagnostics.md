...
effect chain は App runtime が持つ `transmitEffectChainSnapshot` を表示する。Diagnostics は effect の既定値や stage 配列を持たない。VADGate は独立した pipeline 層ではなく、effect chain の 2 つ目の stage として扱う。Diagnostics では FX Chain 行の下へ stage chip をインライン表示し、TX bus の内包関係が見えるようにする。
