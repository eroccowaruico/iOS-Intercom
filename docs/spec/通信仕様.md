...
| ペイロード | `AudioPacketSequencer`, `AudioPacketCodec`, `MultipeerPayloadBuilder` | voice envelope 化、符号化、control payload 化 | 音声と制御を別 payload として扱う |
...
| 音声付随 metadata | codec, fallback, frame 識別 metadata は `ControlMessage.audioFrameMetadata` として Control Plane で送る | 音声 frame には再生に必要な情報だけを残す |
...
| `audioFrameMetadata` | unreliable | codec / fallback / frame 識別 metadata | 音声 frame に不要な送信メタデータを Control Plane へ分離する |
